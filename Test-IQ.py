...
class StanfordBinetTest:
    ...
    def load_60_questions(self):
        """Загружает 60 вопросов для оценки готовности ребенка к школе"""

        questions_raw = [
            # ============= РАЗДЕЛ 1: ОБЩИЕ ЗНАНИЯ ОБ ОКРУЖАЮЩЕМ МИРЕ (10 вопросов) =============
            (2, "Какое время года наступает после зимы?", "Весна", ["Весна", "Лето", "Осень", "Зима"], "Знания"),
            (2, "Сколько дней в неделе?", "7", ["7", "5", "6", "8"], "Знания"),
            (2, "Как называется наша планета?", "Земля", ["Земля", "Марс", "Венера", "Юпитер"], "Знания"),
            (2, "Кто лечит людей?", "Врач", ["Врач", "Учитель", "Строитель", "Повар"], "Знания"),
            (2, "Какой транспорт летает в небе?", "Самолет", ["Самолет", "Поезд", "Машина", "Корабль"], "Знания"),
            (2, "Что мы едим на завтрак?", "Кашу", ["Кашу", "Суп", "Борщ", "Пельмени"], "Знания"),
            (2, "Какой праздник отмечают 1 сентября?", "День знаний", ["День знаний", "Новый год", "8 Марта", "День Победы"], "Знания"),
            (2, "Что помогает нам видеть?", "Глаза", ["Глаза", "Уши", "Нос", "Рот"], "Знания"),
            (2, "Где живут рыбы?", "В воде", ["В воде", "На земле", "В воздухе", "Под землей"], "Знания"),
            (2, "Что нужно для рисования?", "Карандаш", ["Карандаш", "Молоток", "Ножницы", "Иголка"], "Знания"),

            # ============= РАЗДЕЛ 2: МАТЕМАТИЧЕСКИЕ СПОСОБНОСТИ (10 вопросов) =============
            (3, "Сколько будет 2 + 2?", "4", ["4", "3", "5", "6"], "Математика"),
            (3, "Сколько будет 5 - 3?", "2", ["2", "3", "4", "1"], "Математика"),
            (3, "Какое число больше: 7 или 4?", "7", ["7", "4", "Одинаковые", "Не знаю"], "Математика"),
            (3, "Сколько углов у квадрата?", "4", ["4", "3", "5", "6"], "Математика"),
            (3, "Продолжи ряд: 1, 2, 3, 4, ?", "5", ["5", "6", "7", "8"], "Математика"),
            (3, "Сколько будет 3 + 4?", "7", ["7", "6", "8", "9"], "Математика"),
            (3, "Какое число пропущено: 2, 4, 6, ?, 10", "8", ["8", "7", "9", "5"], "Математика"),
            (3, "Сколько будет 10 - 6?", "4", ["4", "5", "3", "6"], "Математика"),
            (3, "Что длиннее: 5 см или 10 см?", "10 см", ["10 см", "5 см", "Одинаково", "Не знаю"], "Математика"),
            (3, "Сколько месяцев в году?", "12", ["12", "10", "11", "13"], "Математика"),

            # ============= РАЗДЕЛ 3: РАЗВИТИЕ РЕЧИ И ГРАМОТА (10 вопросов) =============
            (4, "Найди слово, которое начинается на букву 'М'", "Мама", ["Мама", "Папа", "Дом", "Кот"], "Развитие речи"),
            (4, "Какое слово противоположно слову 'БОЛЬШОЙ'?", "Маленький", ["Маленький", "Огромный", "Широкий", "Высокий"], "Развитие речи"),
            (4, "Сколько слогов в слове 'КОШКА'?", "2", ["2", "1", "3", "4"], "Развитие речи"),
            (4, "Какой звук первый в слове 'АРБУЗ'?", "А", ["А", "Р", "Б", "У"], "Развитие речи"),
            (4, "Назови одним словом: яблоко, груша, банан", "Фрукты", ["Фрукты", "Овощи", "Ягоды", "Сладости"], "Развитие речи"),
            (4, "Какое слово лишнее?", "Чашка", ["Стул", "Стол", "Кровать", "Чашка"], "Развитие речи"),
            (4, "Подбери рифму к слову 'КОТ'", "Рот", ["Рот", "Дом", "Лес", "Сыр"], "Развитие речи"),
            (4, "Сколько букв в слове 'МИР'?", "3", ["3", "2", "4", "5"], "Развитие речи"),
            (4, "Как правильно сказать: 'один ...'?", "Стул", ["Стул", "Стулья", "Стулов", "Стулами"], "Развитие речи"),
            (4, "Назови детеныша кошки", "Котенок", ["Котенок", "Щенок", "Цыпленок", "Жеребенок"], "Развитие речи"),

            # ============= РАЗДЕЛ 4: ЛОГИЧЕСКОЕ МЫШЛЕНИЕ (10 вопросов) =============
            (5, "Что лишнее: волк, лиса, заяц, корова?", "Корова", ["Корова", "Волк", "Лиса", "Заяц"], "Логика"),
            (5, "Если Маша выше Пети, а Петя выше Саши, кто самый низкий?", "Саша", ["Саша", "Маша", "Петя", "Не знаю"], "Логика"),
            (5, "Продолжи: утро, день, вечер, ...", "Ночь", ["Ночь", "Утро", "День", "Вечер"], "Логика"),
            (5, "Что общего у яблока и груши?", "Это фрукты", ["Это фрукты", "Это овощи", "Это сладости", "Это ягоды"], "Логика"),
            (5, "Найди лишнее: береза, дуб, роза, сосна", "Роза", ["Роза", "Береза", "Дуб", "Сосна"], "Логика"),
            (5, "Если у стола отрезать один угол, сколько станет углов?", "5", ["5", "3", "4", "6"], "Логика"),
            (5, "Что тяжелее: 1 кг пуха или 1 кг железа?", "Одинаково", ["Одинаково", "Пух", "Железо", "Не знаю"], "Логика"),
            (5, "Продолжи ряд: понедельник, вторник, среда, ...", "Четверг", ["Четверг", "Пятница", "Суббота", "Воскресенье"], "Логика"),
            (5, "В какую сторону катится колесо, если машина едет вперед?", "Вперед", ["Вперед", "Назад", "Не катится", "Вверх"], "Логика"),
            (5, "Что можно видеть с закрытыми глазами?", "Сон", ["Сон", "Книгу", "Фильм", "Улицу"], "Логика"),

            # ============= РАЗДЕЛ 5: ПРОСТРАНСТВЕННОЕ МЫШЛЕНИЕ (5 вопросов) =============
            (6, "Какая фигура имеет три стороны?", "Треугольник", ["Треугольник", "Квадрат", "Круг", "Прямоугольник"], "Пространство"),
            (6, "Что находится справа от дома, если смотреть на него спереди?", "Неизвестно", ["Неизвестно", "Садик", "Лес", "Река"], "Пространство"),
            (6, "Какая фигура не имеет углов?", "Круг", ["Круг", "Квадрат", "Треугольник", "Прямоугольник"], "Пространство"),
            (6, "Сколько сторон у квадрата?", "4", ["4", "3", "5", "6"], "Пространство"),
            (6, "Что выше: дерево или куст?", "Дерево", ["Дерево", "Куст", "Одинаково", "Зависит от дерева"], "Пространство"),

            # ============= РАЗДЕЛ 6: ПАМЯТЬ И ВНИМАНИЕ (5 вопросов) =============
            (7, "Запомни: яблоко, книга, ручка. Назови второй предмет", "Книга", ["Книга", "Яблоко", "Ручка", "Стол"], "Память"),
            (7, "Запомни числа: 2, 5, 8, 3. Какое число третье?", "8", ["8", "2", "5", "3"], "Память"),
            (7, "Если сейчас 10 часов, сколько будет через 2 часа?", "12", ["12", "11", "13", "10"], "Память"),
            (7, "Запомни: собака, кошка, мышь, корова. Какое животное было вторым?", "Кошка", ["Кошка", "Собака", "Мышь", "Корова"], "Память"),
            (7, "Что было раньше: яйцо или курица?", "Яйцо", ["Яйцо", "Курица", "Оба", "Никто не знает"], "Память"),

            # ============= РАЗДЕЛ 7: ПОДГОТОВКА К ШКОЛЕ (10 вопросов) =============
            (8, "Что нужно взять в школу?", "Ручку", ["Ручку", "Игрушку", "Конфету", "Мяч"], "Школа"),
            (8, "Как нужно вести себя на уроке?", "Слушать учителя", ["Слушать учителя", "Играть", "Разговаривать", "Бегать"], "Школа"),
            (8, "Что делают на перемене?", "Отдыхают", ["Отдыхают", "Учатся", "Едят", "Спят"], "Школа"),
            (8, "Кто учит детей в школе?", "Учитель", ["Учитель", "Врач", "Повар", "Шофер"], "Школа"),
            (8, "Что такое отметка?", "Оценка знаний", ["Оценка знаний", "Запись в дневнике", "Награда", "Подарок"], "Школа"),
            (8, "Какая отметка лучшая?", "5", ["5", "2", "3", "4"], "Школа"),
            (8, "Что нужно сделать перед уроком?", "Приготовить учебники", ["Приготовить учебники", "Пойти гулять", "Посмотреть телевизор", "Пообедать"], "Школа"),
            (8, "Зачем нужен портфель?", "Для книг и тетрадей", ["Для книг и тетрадей", "Для игрушек", "Для еды", "Для одежды"], "Школа"),
            (8, "Кто такие одноклассники?", "Ученики одного класса", ["Ученики одного класса", "Братья и сестры", "Друзья во дворе", "Родственники"], "Школа"),
            (8, "Что такое каникулы?", "Отдых от учебы", ["Отдых от учебы", "Время уроков", "Школьный день", "Учебный год"], "Школа"),
        ]
...
Вам будет предложено 60 вопросов подряд."""
        ...
        # Пример
        tk.Label(form, text="Например: 7", font=("Arial", 11),
    ...
    def show_question(self):
        ...
        if self.current_question_index < len(self.all_questions) and self.test_active:
            ...
            if q['level'] == 2:
                ...
                age_category = "6-7 лет"
            elif q['level'] == 7:
                age_category = "7-8 лет"
            elif q['level'] == 8:
                age_category = "7-8 лет"
            else:
                age_category = f"{q['level']} лет"

            # Показываем категорию и возраст
            self.info_label.config(
                text=f"📚 {q['factor']} | Возраст: {age_category}"
            )
